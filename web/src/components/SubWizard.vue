  ...
  <div class="wizard-overlay" @click.self="$emit('close')">
    <div class="wizard">
      ...
      <div class="wizard-head">
        <div>
          <div class="wizard-title">{{ sub ? '编辑订阅配置' : '新建订阅配置' }}</div>
      ...
      <div class="wizard-body">
        ...
        <template v-if="step === 0">
          <div class="wfield">
            ...
            <input class="input" v-model="form.name" placeholder="例如：我的机场" />
          </div>
          <div class="wfield">
            <div class="wlabel">订阅 URL <span style="color:var(--red)">*</span></div>
            <input class="input input-mono" v-model="form.url" placeholder="https://..." />
            <div class="whint">支持 sing-box JSON 或 Base64 编码分享链接</div>
          </div>
        </template>

        <!-- ═══ Step 2: 通用 ═══════════════════════════════════════════ -->
        <template v-if="step === 1">
          <div class="wsep">日志</div>
          <div class="wfield">
            <div class="wlabel">日志级别</div>
            <div class="seg">
              <button v-for="lv in LOG_LEVELS" :key="lv.v" class="seg-btn"
                :class="{ on: form.log.level === lv.v }" :disabled="form.log.disabled"
                @click="form.log.level = lv.v">{{ lv.l }}</button>
            </div>
          </div>
          <div class="wfield winline">
            <div class="toggle" :class="{ on: form.log.disabled }" @click="form.log.disabled=!form.log.disabled"></div>
            <span class="wlabel" style="margin:0">禁用日志输出</span>
          </div>
          <div class="wfield winline">
            <div class="toggle" :class="{ on: form.log.timestamp }" @click="form.log.timestamp=!form.log.timestamp"></div>
            <span class="wlabel" style="margin:0">日志时间戳</span>
          </div>
          <div class="wfield">
            <div class="wlabel">日志路径（留空不保存）</div>
            <input class="input input-mono" v-model="form.log.output" placeholder="/var/log/sing-box.log" />
          </div>

          <div class="wsep">Clash API</div>
          <div class="wfield">
            <div class="wlabel">监听地址</div>
            <input class="input input-mono" v-model="form.clashAPI.external_controller" placeholder="127.0.0.1:9090" />
          </div>
          <div class="wfield">
            <div class="wlabel">API 密钥</div>
            <div style="display:flex;gap:8px">
              <input class="input input-mono" v-model="form.clashAPI.secret" placeholder="留空不需要密钥" style="flex:1" />
              <button class="btn btn-ghost btn-sm" @click="form.clashAPI.secret=genSecret()">随机</button>
            </div>
          </div>
          <div class="wfield">
            <div class="wlabel">外部 UI 下载地址</div>
            <input class="input input-mono" v-model="form.clashAPI.external_ui_download_url" placeholder="留空使用 zashboard" />
          </div>
          <div class="wfield">
            <div class="wlabel">默认模式</div>
            <div class="seg">
              <button v-for="m in CLASH_MODES" :key="m.v" class="seg-btn" :class="{ on: form.clashAPI.default_mode===m.v }" @click="form.clashAPI.default_mode=m.v">{{ m.l }}</button>
            </div>
          </div>
          <div class="wfield winline">
            <div class="toggle" :class="{ on: form.clashAPI.access_control_allow_private_network }"
              @click="form.clashAPI.access_control_allow_private_network=!form.clashAPI.access_control_allow_private_network"></div>
            <span class="wlabel" style="margin:0">允许局域网访问 API</span>
          </div>

          <div class="wsep">缓存文件</div>
          <div class="wfield winline">
            <div class="toggle" :class="{ on: form.cacheFile.enabled }" @click="form.cacheFile.enabled=!form.cacheFile.enabled"></div>
            <span class="wlabel" style="margin:0">启用缓存</span>
          </div>
          <div class="wfield">
            <div class="wlabel">缓存路径</div>
            <input class="input input-mono" v-model="form.cacheFile.path" placeholder="cache.db" />
          </div>
          <div class="wfield winline">
            <div class="toggle" :class="{ on: form.cacheFile.store_fakeip }" @click="form.cacheFile.store_fakeip=!form.cacheFile.store_fakeip"></div>
            <span class="wlabel" style="margin:0">存储 FakeIP 映射</span>
          </div>
        </template>
        ...
        <template v-if="step === 2">
          <div style="display:flex;gap:8px;margin-bottom:12px;flex-wrap:wrap">
            <button v-for="tp in INBOUND_TYPES" :key="tp.v" class="btn btn-ghost btn-sm" @click="addInbound(tp.v)">
              + {{ tp.l }}
            </button>
          </div>

          <div v-if="!form.inbounds.length" class="empty">点击上方按钮添加入站</div>

          <!-- draggable inbound list -->
          <div ref="ibDragEl">
            <div v-for="(ib, idx) in form.inbounds" :key="ib.id" class="ib-card"
              style="margin-bottom:10px"
              :data-id="ib.id">
              <div class="ib-head">
                <span class="drag-handle" style="cursor:move;color:var(--text3);margin-right:4px">⠿</span>
                <span class="ib-tag">{{ ib.tag }}</span>
                <span class="ib-badge">{{ ib.type }}</span>
                <div class="toggle" :class="{ on: ib.enable }" @click="ib.enable=!ib.enable" style="margin-left:auto"></div>
                <button class="btn-icon danger" style="margin-left:6px" @click="form.inbounds.splice(idx,1)">✕</button>
              </div>

              <template v-if="ib.enable">
                <div class="wfield">
                  <div class="wlabel">Tag 名称</div>
                  <input class="input" v-model="ib.tag" />
                </div>
                ...
                <template v-if="ib.type !== 'tun'">
                  <div class="wfield">
                    <div class="wlabel">监听地址</div>
                    <input class="input input-mono" v-model="ib.listen" />
                  </div>
                  <div class="wfield">
                    <div class="wlabel">端口</div>
                    <input class="input" type="number" v-model.number="ib.listen_port" />
                  </div>
                  <div class="wfield">
                    <div class="wlabel">认证用户（user:pass，每行一个）</div>
                    <textarea class="textarea" v-model="ib.usersText" rows="2" placeholder="user:password"></textarea>
                  </div>
                  <div style="display:flex;flex-direction:column;gap:8px">
                    <label class="winline" style="cursor:pointer">
                      <div class="toggle" :class="{ on: ib.tcp_fast_open }" @click="ib.tcp_fast_open=!ib.tcp_fast_open"></div>
                      <span style="font-size:13px">TCP 快速打开</span>
                    </label>
                    <label class="winline" style="cursor:pointer">
                      <div class="toggle" :class="{ on: ib.tcp_multi_path }" @click="ib.tcp_multi_path=!ib.tcp_multi_path"></div>
                      <span style="font-size:13px">多路径 TCP</span>
                    </label>
                    <label class="winline" style="cursor:pointer">
                      <div class="toggle" :class="{ on: ib.udp_fragment }" @click="ib.udp_fragment=!ib.udp_fragment"></div>
                      <span style="font-size:13px">UDP 分段</span>
                    </label>
                  </div>
                  <!-- redirect / tproxy extra -->
                  <template v-if="ib.type === 'redirect' || ib.type === 'tproxy'">
                    <div class="wfield" style="margin-top:8px">
                      <div class="wlabel">路由标签（转发给哪个出站）</div>
                      <input class="input" v-model="ib.route_address" placeholder="例如 0.0.0.0/1" />
                    </div>
                  </template>
                </template>
                ...
                <template v-else>
                  <div class="wfield">
                    <div class="wlabel">TUN 网卡名称（留空自动）</div>
                    <input class="input" v-model="ib.interface_name" placeholder="自动" />
                  </div>
                  <div class="wfield">
                    <div class="wlabel">地址（逗号分隔）</div>
                    <input class="input input-mono" v-model="ib.addressText" placeholder="172.18.0.1/30, fdfe:dcba:9876::1/126" />
                  </div>
                  <div class="wfield">
                    <div class="wlabel">MTU（0=自动）</div>
                    <input class="input" type="number" v-model.number="ib.mtu" />
                  </div>
                  <div class="wfield">
                    <div class="wlabel">TUN 模式堆栈</div>
                    <div class="seg">
                      <button v-for="s in TUN_STACKS" :key="s" class="seg-btn" :class="{ on: ib.stack===s }" @click="ib.stack=s">{{ s }}</button>
                    </div>
                  </div>
                  <div style="display:flex;flex-direction:column;gap:8px;margin-top:6px">
                    <label class="winline" style="cursor:pointer">
                      <div class="toggle" :class="{ on: ib.auto_route }" @click="ib.auto_route=!ib.auto_route"></div>
                      <span style="font-size:13px">自动路由 auto_route</span>
                    </label>
                    <label class="winline" style="cursor:pointer">
                      <div class="toggle" :class="{ on: ib.strict_route }" @click="ib.strict_route=!ib.strict_route"></div>
                      <span style="font-size:13px">严格路由 strict_route</span>
                    </label>
                    <label class="winline" style="cursor:pointer">
                      <div class="toggle" :class="{ on: ib.endpoint_independent_nat }" @click="ib.endpoint_independent_nat=!ib.endpoint_independent_nat"></div>
                      <span style="font-size:13px">端点无关 NAT</span>
                    </label>
                  </div>
                  <div class="wfield" style="margin-top:8px">
                    <div class="wlabel">route_address（留空不设）</div>
                    <input class="input input-mono" v-model="ib.route_address_text" placeholder="0.0.0.0/1, 128.0.0.0/1" />
                  </div>
                  <div class="wfield">
                    <div class="wlabel">route_exclude_address</div>
                    <input class="input input-mono" v-model="ib.route_exclude_address_text" placeholder="192.168.0.0/16" />
                  </div>
                </template>
              </template>
            </div>
        ...
        <template v-if="step === 4">
          ...
          <div v-if="routeTab==='general'">
            <div class="wfield winline">
              <div class="toggle" :class="{ on: form.route.find_process }" @click="form.route.find_process=!form.route.find_process"></div>
              <div>
                <div class="wlabel" style="margin:0">查找进程信息</div>
                <div class="whint">find_process — 性能消耗较大</div>
              </div>
            </div>
            <div class="wfield winline" style="margin-top:10px">
          ...
          <div v-if="routeTab==='rules'">
            ...
            <div class="whint" style="margin-bottom:8px">按顺序匹配，点击行切换启用/禁用，拖动 ⠿ 调整顺序</div>
            ...
            <div ref="routeRuleDragEl" style="border:1px solid var(--border);border-radius:var(--radius);overflow:hidden">
              <div v-for="(rule, idx) in form.route.rules" :key="rule.id"
                class="rule-row" :class="{ off: !rule.enable }"
                @click="rule.type!=='InsertionPoint' && (rule.enable=!rule.enable)">

                <!-- Insertion point -->
                <div v-if="rule.type==='InsertionPoint'"
                  style="width:100%;text-align:center;padding:4px 0;background:var(--accent-bg);color:var(--accent);font-size:11px;font-weight:700;cursor:move"
                  @click.stop>
                  <span class="drag-handle">⠿</span> + 新规则插入到这里
                </div>

                <template v-else>
                  <span class="drag-handle" style="cursor:move;color:var(--text3);flex-shrink:0" @click.stop>⠿</span>
        ...
        <template v-if="step === 5">
          ...
          <div v-if="dnsTab==='rules'">
            ...
            <div class="whint" style="margin-bottom:8px">点击行切换启用/禁用，拖动 ⠿ 调整顺序</div>
            <div v-if="!form.dns.rules.length" class="empty">暂无 DNS 规则</div>
            <div ref="dnsRuleDragEl" style="border:1px solid var(--border);border-radius:var(--radius);overflow:hidden">
  ...
  <div v-if="showRouteRuleModal" class="mask" @click.self="showRouteRuleModal=false" style="z-index:500">
    <div class="modal" style="max-width:640px;max-height:88vh;overflow-y:auto">
      ...
      <div class="modal-body">
        ...
        <!-- payload -->
        <div v-if="rrFields.type !== 'rule_set'" class="wfield">
...
import { useSubsStore, useRulesetsStore } from '../stores.js'

const props = defineProps({ sub: { type: Object, default: null } })
...
// ── Constants ──────────────────────────────────────────────────────────────
const stepTitles   = ['名称设置','通用设置','入站设置','出站设置','路由设置','DNS 设置']
const LOG_LEVELS   = [
...
const RULE_TYPES = [
  {v:'inbound',l:'inbound'},{v:'network',l:'network'},{v:'protocol',l:'protocol'},
...
function defaultRouteRules() {
  ...
  return [
    { id:uid(), type:'inbound',    enable:true,  payload:'tun-in',          action:'sniff',      outbound:'', invert:false, sniffer:[], strategy:'default', server:'' },
    { id:uid(), type:'protocol',   enable:true,  payload:'dns',             action:'hijack-dns', outbound:'', invert:false, sniffer:[], strategy:'default', server:'' },
    ...
    { id:uid(), type:'network',    enable:true,  payload:'icmp',            action:'route',      outbound:OB_IDS.DIRECT,   invert:false, sniffer:[], strategy:'default', server:'' },
    { id:uid(), type:'protocol',   enable:true,  payload:'quic',            action:'route',      outbound:OB_IDS.BLOCK,    invert:false, sniffer:[], strategy:'default', server:'' },
...
function buildForm() {
  return {
    name: '', url: '',
    log:      { disabled:false, level:'warn', output:'', timestamp:true },
    clashAPI: { external_controller:'127.0.0.1:9090', external_ui:'ui', external_ui_download_url:'', external_ui_download_detour:'direct', secret:'', default_mode:'rule', access_control_allow_origin:['*'], access_control_allow_private_network:false },
    cacheFile:{ enabled:true, path:'cache.db', store_fakeip:true },
...
onMounted(async () => {
  await rsStore.scan()
  if (!props.sub) return
  form.name = props.sub.name || ''
  form.url  = props.sub.url  || ''
  const wc = props.sub.wizardConfig
  if (!wc) return
  const src = typeof wc === 'string' ? JSON.parse(wc) : wc
  if (src.log)       Object.assign(form.log, src.log)
  if (src.clashAPI)  Object.assign(form.clashAPI, src.clashAPI)
  if (src.cacheFile) Object.assign(form.cacheFile, src.cacheFile)
  if (src.inbounds?.length) form.inbounds = JSON.parse(JSON.stringify(src.inbounds))
...
function initDrag(el, list) {
  if (!el) return
  let dragIdx = null
  el.querySelectorAll('[data-drag]').forEach((item, i) => {
    item.draggable = true
...
// Simple drag via CSS class; attach on render
...
// Make lists directly sortable with mouse events (simpler than HTML5 drag API)
function useSortable(listRef, listData) {
  watch([listRef, () => listData.value?.length ?? listData.length], () => {
    nextTick(() => {
      ...
      function onMove(e) {
        ...
        for (let j = 0; j < els.length; j++) {
          ...
          if (e.clientY >= r.top && e.clientY <= r.bottom && j !== dragging) {
            over = j; break
          }
        }
      }
      function onUp() {
        if (dragging !== null && over !== null && over !== dragging) {
          const arr = Array.isArray(listData) ? listData : listData.value
          const moved = arr.splice(dragging, 1)[0]
          arr.splice(over, 0, moved)
        }
        if (dragging !== null) items()[dragging].style.opacity = '1'
...
useSortable(routeRuleDragEl, form.route.rules)
useSortable(dnsRuleDragEl,   form.dns.rules)
